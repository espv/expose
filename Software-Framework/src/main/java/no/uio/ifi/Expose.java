	...
	String doHandleEvent(String cmd, List<Object> args, int node_id) {
		...
		switch (cmd) {
			case "setEventBatchSize": {
				ret = this.mc.nodeIdsToExperimentAPIs.get(node_id).SetTupleBatchSize((int) args.get(0));
				break;
			}
			case "wait": {
				if (args.size() == 0) {
			...
			case "runEnvironment": {
				ret = this.mc.nodeIdsToExperimentAPIs.get(node_id).RunEnvironment();
				break;
			}
			case "stopEnvironment": {
				ret = this.mc.nodeIdsToExperimentAPIs.get(node_id).StopEnvironment();
				break;
			}
			case "setIntervalBetweenEvents": {
				ret = this.mc.nodeIdsToExperimentAPIs.get(node_id).SetIntervalBetweenTuples((int) args.get(0));
				break;
			}
			case "addEvents": {
				...
				ret = this.mc.nodeIdsToExperimentAPIs.get(node_id).AddTuples(spe_event_to_create, (int) args.get(1));
				break;
			}
			case "addDataset": {
			...
			case "addQueries": {
				...
				for (int i = 0; i < quantity; ++i) {
					ret = this.mc.nodeIdsToExperimentAPIs.get(node_id).AddQueries(spe_rule_to_create);
				}
				break;
			}
			case "loopTasks": {
				...
				ret = this.mc.nodeIdsToExperimentAPIs.get(node_id).ProcessTuples((int) args.get(0));
				break;
			}
			case "processDataset": {
				int dataset_id = (int) args.get(0);
				...
				for (Map<String, Object> ds : datasets) {
					...
					if ((int) ds.get("id") == dataset_id) {
						ret = this.mc.nodeIdsToExperimentAPIs.get(node_id).ProcessDataset(ds);
						break;
					}
				}
				break;
			...
			case "runParallel": {
				...
				for (Object o : args) {
					...
					this.mc.PrintIncomingOfClient(nodeId);
				}
				break;
			}
			case "clearQueries": {
				tf.traceEvent(222);
				ret = this.mc.nodeIdsToExperimentAPIs.get(node_id).ClearQueries();
				break;
			}
			case "clearEvents": {
				tf.traceEvent(223);
				ret = this.mc.nodeIdsToExperimentAPIs.get(node_id).ClearTuples();
				break;
			}
			case "setNodeIdToAddress": {
				Map<Integer, Map<String, Object> > nodeIdToIpAndPort = (Map<Integer, Map<String, Object> >) args.get(0);
				ret = this.mc.nodeIdsToExperimentAPIs.get(node_id).SetNodeIdToAddress(nodeIdToIpAndPort);
				break;
			}
			case "cleanupExperiment": {
				ret = this.mc.nodeIdsToExperimentAPIs.get(node_id).CleanupExperiment();
				break;
			}
			case "addTracepointIds": {
				ret = this.mc.nodeIdsToExperimentAPIs.get(node_id).AddTracepointIds(args);
				break;
			}
			case "notifyAfterNoReceivedTuple": {
				ret = this.mc.nodeIdsToExperimentAPIs.get(node_id).NotifyAfterNoReceivedTuple((int) args.get(0));
				System.out.println("Last received tuple: " + ret + " ms ago");
				break;
			}
			case "traceTuple": {
				ret = this.mc.nodeIdsToExperimentAPIs.get(node_id).TraceTuple((int) args.get(0), (List<String>) args.get(1));
				break;
			}
			case "configure": {
				ret = this.mc.nodeIdsToExperimentAPIs.get(node_id).Configure();
				break;
			}
			default: {
